...
public:
	...
	void PatchBytes(const std::vector<uint8_t> &vOpcodeArray) const
	{
	...
	void NOP(const size_t nSize) const
	{
		...
		for (size_t i = 0; i < nSize; i++)
		{
			*reinterpret_cast<uint8_t *>(ptr + i) = 0x90;
		}
	...
	CMemory FindPattern(const char *const szPattern, const Direction searchDirect = Direction::DOWN, const int opCodesToScan = 512, const ptrdiff_t nOccurences = 1) const
	{
		...
		const std::vector<int> PatternBytes = silverbun::PatternToBytes(szPattern);
	...
	CMemory FindPatternSelf(const char *const szPattern, const Direction searchDirect = Direction::DOWN, const int opCodesToScan = 512, const ptrdiff_t occurrence = 1)
	{
		...
		const std::vector<int> PatternBytes = silverbun::PatternToBytes(szPattern); // Convert our pattern to a byte array.
		const std::pair<size_t, const int *> bytesInfo = std::make_pair<size_t, const int *>(PatternBytes.size(), PatternBytes.data());
		ptrdiff_t occurrences = 0;
...
public:
	...
	{
		const std::pair<std::vector<uint8_t>, std::string> patternInfo = silverbun::PatternToMaskedBytes(szPattern);
	...
	CMemory FindStringReadOnly(const char *const szString, const bool bNullTerminator) const
	{
		...
		const std::vector<int> vBytes = silverbun::StringToBytes(szString, bNullTerminator);
	...
	CMemory GetVirtualMethodTable(const char *const szTableName, const size_t nRefIndex = 0) const
	{
		...
		const auto tableNameInfo = silverbun::StringToMaskedBytes(szTableName, false);
	...
	inline CMemory Offset(ptrdiff_t diff) const { return CMemory(m_pModuleBase).OffsetSelf(diff); }
